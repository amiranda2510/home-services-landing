            ...
            <div className="py-32">
                <div className="mx-auto max-w-5xl px-6">
                    <div className="text-center">
                        ...
                        <p className="mt-6 font-semibold">Reparaciones, mejoras y mantenimiento con responsabilidad y buen acabado.</p>
                    </div>
...
const IntegrationCard = ({ title, description, children }: { title: string; description: string; children: React.ReactNode; link?: string }) => {
    return (
        <Card className="group p-6 transition-all duration-350 ease-out hover:-translate-y-1.5 hover:shadow-xl cursor-default bg-muted/50">
            <div className="relative">
                <div className="*:size-10">{children}</div>

                <div className="space-y-2 py-6">
                    <h3 className="text-base font-medium">{title}</h3>
                    <p className="text-muted-foreground line-clamp-2 group-hover:line-clamp-none text-sm transition-all duration-300">{description}</p>
                </div>
